...
test.describe('Pay with Wallet (US2)', () => {
  ...
  test('pays a pending request with wallet', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { level: 1, name: '$10.00' })).toBeVisible({
      timeout: 20000,
    });
  ...
  test('shows insufficient balance when wallet cannot cover amount', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { level: 1, name: '$9,999.00' })).toBeVisible({
      timeout: 20000,
    });
  ...
  test('prevents double-click on pay button via loading state', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { level: 1, name: '$5.00' })).toBeVisible({
      timeout: 20000,
    });
